...
\begin{center}
  \begin{tikzpicture}[
    ...
    % 関数のグラフ
    \draw[domain=.5:5.3,samples=200,variable=\x,violet!50,very thick] plot ({\x},{f(\x)}) node [above right] {$y=f(x)$};
  \end{tikzpicture}
\end{center}
...
\begin{center}
  \begin{tikzpicture}[
    ...
    \begin{scope}[scale=0.9, local bounding box=left]
      ...
      % 関数のグラフ
      \draw[domain=.5:5.3,samples=200,variable=\x,violet!50,very thick] plot ({\x},{f(\x)});
    \end{scope}
