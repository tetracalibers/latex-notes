...
たとえば、斜面にボールを置いたとき、ボールは最大傾斜の方向に転がっていくことは直観的にわかる。

また、高さが一定になる方向は、最大傾斜の方向と垂直であることも直観的に理解している。

\br

高さが一定（$\Delta z = 0$）になる方向は、勾配ベクトル$\nabla f$と垂直であったから、この勾配ベクトル$\nabla f$が最大傾斜の方向を表しているのでは？と予想できる。
...
\begin{center}
  \begin{tikzpicture}
        ...
        % 座標軸
        \draw[axis] (-1.5, -1) -- (3, -1) node[right] {$x$};
        \draw[axis] (-1.5, -1) -- (-1.5, 2.5) node[above] {$y$};

        % 角度
        \pic [fill=Emerald!30,angle eccentricity=1.3, angle radius=2em, "$\theta$"{text=Emerald}] {angle = C--A--B};
        
        % x軸方向のベクトル
        \draw[vector, very thick, Rhodamine] (A) -- (C) node[below, pos=0.4] {$\Delta x$};
        % y軸方向のベクトル
        \draw[vector, very thick, Cerulean] (C) -- (B) node[right, midway] {$\Delta y$};

        % ベクトル
        \draw[thick, BurntOrange] (B) -- (A) node[midway, above left] {$h$};
        
        % (x,y)
        \filldraw[lightslategray] (A) node[below left] {$(x,y)$};
      \end{tikzpicture}
\end{center}
