...
\begin{proof}
  ...
  そして、\thmref{thm:homogeneous-solution-iff-det-zero}より、斉次形方程式が非自明解を持つことは、行列式が0になることと同値である
...
そこで、固有値の\keyword{重複度}を次のように定義する
...
\thmref{thm:determinant-multiplicativity}より、正方行列$A,\,B$が、ある正則行列$P$に対して
\begin{equation*}
  B = P^{-1}AP
\end{equation*}
となる（$A$と$B$が\defref{def:similar-matrices}である）ならば、次のように$A$と$B$の特性多項式は一致する
\begin{align*}
...
さらに、$A$が\defref{def:diagonalizable}であるときには、
\begin{equation*}
...
さて、$A$と$B$が\defref{def:similar-matrices}であるとき、\thmref{thm:similarity-under-basis-change}より、$A$と$B$は1つの線形変換$f$を異なる基底によって表現して得られた行列であるという関係にある
...
また、\thmref{thm:diagonalizable-iff-n-indep-eigenvectors}の証明過程を振り返ると、次のようにまとめられる
