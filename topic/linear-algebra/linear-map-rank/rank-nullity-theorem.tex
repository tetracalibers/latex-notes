...
連立方程式$A\vb*{x}=\vb*{b}$の\secref{sec:degrees-of-freedom}は、
\begin{equation*}
  \text{\bfseries 解の自由度} = (\text{\bfseries 変数の個数}) - \rank A
\end{equation*}
で表された。

そして、\secref{sec:nullity-solution-degrees}より、この解の自由度は$\Ker A$の次元と一致する。
\begin{equation*}
...
さらに、\defref{def:rank-of-linear-map}から、
\begin{equation*}
...
\secref{sec:injectivity-condition-kernel}より、潰れる部分$\Ker A$が$\{ \vb*{o} \}$でないことは、単射ではないことを意味する。
...
\secref{sec:image-and-surjectivity}より、$\Im A$が写り先の空間全体をカバーしていないことは、全射ではないことを意味する。
