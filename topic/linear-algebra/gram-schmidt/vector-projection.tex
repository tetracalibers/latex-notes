...
\secref{sec:inner-product-similarity}は、次のような図で内積の意味を捉えようとするものだった。
...
\defref{def:angle-between-vectors}より、$\vb*{u}, \vb*{v}$のなす角$\theta$を用いると、$\vb*{v}$の$\vb*{u}$との内積は次のように表すことができる。
\begin{equation*}
...
\defref{def:inner-product-axioms}のうち、内積の双線形性を用いて展開すると、
\begin{gather*}
