...
\begin{proof}
  ...
  \thmref{thm:coordinate-change-rule}より、$\vb*{v}$の$\mathcal{U}$に関する座標ベクトルを$\vb*{c} \in \mathbb{C}^n$とすると、
  \begin{equation*}
    \vb*{v} = U \vb*{c}
  \end{equation*}
  両辺に左から$U^*$をかけると、
...
\begin{proof}
  \thmref{thm:unitary-coordinate-transform}より、座標ベクトルは$\vb*{c} = V^* \vb*{v}$で与えられる。
  ...
  ここで、\thmref{thm:unitary-characterized-by-norm-invariance}より、左からユニタリ行列$U$をかけてもノルムは変わらないので、
  \begin{equation*}
  ...
  よってそのノルムの二乗は、\defref{def:standard-inner-product-Cn}より、$\mathbb{C}^n$上の自分自身との内積と考えて、
  \begin{equation*}
