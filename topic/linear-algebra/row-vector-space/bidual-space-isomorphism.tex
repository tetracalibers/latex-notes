...
\begin{handout}[補足：$\Phi_{\vb*{v}}$の線形性]
  ...
  $\phi_1, \phi_2$は線形写像であるので、\defref{def:linear-map-addition-scalar}より、
  \begin{align*}
...
そのためには、\defref{def:linear-isomorphism}より、$\iota$の全単射性を証明できればよい。
...
\begin{proof}
  \thmref{thm:single-vec-indep-iff-nonzero}より、$\vb*{v} \neq \vb*{o}$は線型独立である。

  よって、\thmref{thm:basis-extension}により、$\vb*{v}$を含む$V$の基底$\{ \vb*{v}, \vb*{v}_2, \dots, \vb*{v}_n \}$を選ぶことができる。
...
\begin{proof}
  \begin{subpattern}{\bfseries 写像$\iota$は単射}
    ...
    ここで、$\vb*{v} \neq \vb*{o}$とすると、\thmref{thm:dual-space-separation}より、$\phi(\vb*{v}) \neq 0$となるような線形汎関数$\phi$が存在する。
    ...
    \end{equation*}
    \thmref{thm:injective-zero-test}より、これは線形写像$\iota$が単射であることを示している。$\qed$
  \end{subpattern}
  
  \begin{subpattern}{\bfseries 写像$\iota$は全射}
    \thmref{thm:dual-basis-dimension}を考えると、
    \begin{equation*}
      \dim (V^*)^* = \dim V^* = \dim V
    \end{equation*}

    $\iota$が単射であることから$\Ker(\iota) = \{ \vb*{o} \}$なので、\thmref{thm:rank-nullity-theorem}より、$\dim (V^*)^* = \dim V$は$\iota \colon V \to (V^*)^*$が全射であることを示している。$\qed$
  \end{subpattern}
\end{proof}
