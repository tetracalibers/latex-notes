...
\sectionline
\section{線形写像の記述と行列}\label{sec:matrix-description-linear-map}
%\marginnote{}
...
\subsection{行列：線形写像の簡略記法}\label{sec:matrix-linear-map-notation}
\marginnote{
...
\begin{itemize}
  \item \keyword{実数}を縦横に並べた$m \times n$型行列の集合は、$M_{mn}(\mathbb{R})$と表す。
  \item \keyword{複素数}を縦横に並べた$m \times n$型行列の集合は、$M_{mn}(\mathbb{C})$と表す。
\end{itemize}
