...
\begin{proof}
  $W = \{ \vb*{0} \}$の場合は、任意の$\vb*{v} \in V$に対して$\vb*{0}$との内積は0になることから、$W^\perp$は$V$全体となる
  \begin{equation*}
    W^\perp = \{ \vb*{v} \in V \mid  (\vb*{v}, \vb*{0}) = 0 \} = V
  \end{equation*}
  よって、
  \begin{equation*}
    V = W + W^\perp = \{ \vb*{0} \} + V = V
...
\begin{proof}
  ...
  \end{equation*}
  であり、等号が成立するのは、$\vb*{a} = \vb*{0}$のときのみである

  よって、$\vb*{a} = \vb*{0}$である

  \br

  零ベクトルは任意のベクトルと直交し（内積が0になり）、また任意の部分空間に属するので、明らかに$\vb*{0} \in W \cap W^\perp$である

  \br

  $\vb*{a}$は$W \cap W^\perp$の任意の元であり、$\vb*{a} = \vb*{0} \in W \cap W^\perp$であることがわかったので、
  \begin{equation*}
    W \cap W^\perp = \{ \vb*{0} \}
  \end{equation*}
  がいえる $\qed$
...
こうして、次の\hyperref[thm:direct-sum-equiv]{両方が成り立つことから}、
\begin{enumerate}[label=\romanlabel]
  \item $V = W + W^\perp$
  \item $W \cap W^\perp = \{ \vb*{0} \}$
\end{enumerate}
計量空間$V$は部分空間$W$とその直交補空間$W^\perp$の直和として分解できる

\br

\hyperref[thm:dim-direct-sum]{直和の次元公式}より、次の定理が従う
