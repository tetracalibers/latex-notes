...
$\mathbb{R}^n$上の内積は、2つのベクトル$\vb*{a}, \vb*{b} \in \mathbb{R}^n$のペア（\defref{def:cartesian-product}）から、スカラー値$\mathbb{R}$を返す関数として捉えることができる。
...
\begin{proof}
  \begin{subpattern}{{\bfseries 和に対する双線形性} (\romannum{i})}
    \thmref{thm:transpose-distributes-over-sum}より、
    \begin{align*}
  ...
  \begin{subpattern}{{\bfseries スカラー倍に対する双線形性} (\romannum{iii})}
    \thmref{thm:transpose-of-product}と、スカラー（$1 \times 1$型行列）を転置しても変わらないことを用いて、
    \begin{align*}
