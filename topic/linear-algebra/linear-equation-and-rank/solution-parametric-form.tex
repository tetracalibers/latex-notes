...
\begin{definition}{主変数と自由変数}
  行列$A$を行基本変形により行階段形にしたとき、\hyperref[sec:def-pivot]{主成分}がある列に対応する変数を\keyword{主変数}と呼び、それ以外の変数を\keyword{自由変数}と呼ぶ。
\end{definition}
...
また、パラメータをかけていない列ベクトル$\vb*{b}'$は、連立方程式の定数項$\vb*{b}$に行基本変形を施した結果として得られるベクトルであり、これを\keyword{特殊解}という。
