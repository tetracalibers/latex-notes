...
\begin{theorem}{正規行列における固有空間の次元と固有値の重複度の一致}{normal-matrix-eigenvalue-multiplicity}
  ...
  \thmref{thm:rank-nullity-theorem}を用いて言い換えると、$\alpha$の固有空間$W(\alpha)$について、
  \begin{equation*}
    \dim W(\alpha) = k
  \end{equation*}
  が成り立つ
...
\begin{proof}
  ...
  左辺は、\thmref{thm:adjoint-of-product}より、積の随伴行列をつくると積の順序が入れ替わることに注意して、
  \begin{equation*}
    (U^*AU)^* = U^* A^* (U^*)^* = U^* A^* U
  \end{equation*}
  ...
  ここで、\thmref{thm:char-poly-of-similar-matrices}より、$A$と$U^*AU$の特性多項式は一致するので、実際に計算すると、
  \begin{align*}
  ...
  \end{equation*}
  ここで、$A$と$U^*AU$は相似な行列であり、\thmref{thm:eigenvalues-of-similar-matrices}より、相似な行列の固有値（特性方程式の根）は重複度も含めて一致するので、
  \begin{equation*}
  ...
  \end{equation*}
  $\alpha E_{n-l} - C$は行列の階数が次数$n-l$に等しいので、\thmref{thm:invertible-iff-full-rank}より、正則行列である
...
\thmref{thm:diagonalizable-iff-eigenspace-dim-equals-multiplicity}より、正規行列は対角化可能である
...
\begin{proof}
  \begin{subpattern}{\bfseries 正規行列 $\Longrightarrow$ ユニタリ行列を用いて対角化可能}
    \thmref{thm:normal-matrix-eigenvalue-multiplicity}の定理の証明過程より明らか $\qed$
  \end{subpattern}
  ...
  \begin{subpattern}{\bfseries ユニタリ行列を用いて対角化可能 $\Longrightarrow$ 正規行列}
    ...
    よって、$A^*$は、\thmref{thm:adjoint-of-product}より、積の随伴行列をつくると積の順序が入れ替わることに注意して、
    \begin{align*}
      A^* & = (U^*)^* \begin{pNiceArray}{ccc}
                        \overline{\alpha_1} & & O \\
                        &  \ddots & \\
