...
\thmref{thm:transpose-involution}より、転置を二回行うと元に戻る。
...
\defref{def:standard-inner-product-Cn}は、随伴を用いて表現することもできる
...
\begin{proof}
  \thmref{thm:standard-inner-product-symmetry}より、
  \begin{equation*}
    (\vb*{a},\vb*{b}) = \overline{(\vb*{b},\vb*{a})}
  \end{equation*}

  ここで、\thmref{thm:inner-product-as-transpose-product}より、右辺の内積を転置を用いて表すと、
  \begin{equation*}
...
随伴行列と\defref{def:standard-inner-product-Cn}は、次のような関係で結ばれる
...
\begin{proof}
  \thmref{thm:inner-product-as-transpose-product}より、転置を用いて内積を表すと、
  \begin{equation*}
    (A\vb*{u},\vb*{v}) = {}^t(A\vb*{u}) \overline{\vb*{v}}
  \end{equation*}
  \thmref{thm:transpose-of-product}より、${}^t(A\vb*{u}) = {}^t\vb*{u} \transpose{A}$なので、
  \begin{equation*}
