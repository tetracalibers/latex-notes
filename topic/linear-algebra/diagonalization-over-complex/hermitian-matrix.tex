...
\end{equation*}
となり、このような$A$は\defref{def:symmetric-matrix}、あるいは\keyword{実対称行列}と呼ばれる
...
\begin{proof}
  ...
  一方、\thmref{thm:adjoint-identity}から、次のようにも書ける
  \begin{equation*}
  ...
  \begin{align*}
    (A\vb*{v}, \vb*{v}) & = (\vb*{v}, A\vb*{v})      \\
                        & = (\vb*{v}, \alpha\vb*{v})
  \end{align*}
  \thmref{thm:conjugate-linearity-of-inner-product}に注意して、
  \begin{equation*}
...
\begin{proof}
  ...
  一方、\thmref{thm:adjoint-identity}より、
  \begin{equation*}
  ...
  \end{equation*}
  ここで、$\alpha,\,\beta$は実数なので、\thmref{thm:conjugate-linearity-of-inner-product}を考慮しても、
  \begin{equation*}
...
\end{equation*}
とおけば、\thmref{thm:unitary-iff-columns-orthonormal}より、$U$はユニタリ行列となる
...
さらに、ユニタリ行列はその\thmref{def:unitary-matrix}から明らかに正則行列であるので、\thmref{thm:diagonalization-columns-are-eigenvectors}を振り返ると、
\begin{shaded}
