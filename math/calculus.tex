...
ここで、下限$a$は固定し、上限$b$を変化させて積分区間を広げていくことを考えよう。

このとき、定積分$\displaystyle\int_{a}^{b}f(x)dx$は、$b$の関数として捉えられる。
...
\begin{center}
  \begin{tikzpicture}[
    ...
    % 積分区間[a,b]
    \draw[fill=lightgray!20, draw=lightgray!80!gray] plot[domain=\a:\b,samples=167,variable=\x] ({\x},{f(\x)}) -- (\b,0) -| cycle;
    % ↑の領域の中央に、定積分の式のラベルを配置
    \node[gray] at ({(\a+\b)/2},{f((\a+\b)/2)/2}) {$\displaystyle\int_{a}^{b}f(x)dx$};

    % [b, b+db]
    \draw[fill=magenta!30, draw=magenta!70!gray] plot[domain=\b:\b+\db,samples=167,variable=\x] ({\x},{f(\x)}) -- (\b+\db,0) -| cycle;
    ...
    % b+dbのラベル
    \node[anchor=north,lnode, below right, xshift=-0.5em] at (\b+\db,0) {$b+\Delta b$};

    % (b, f(b))の点
    \fill[magenta] (\b,{f(\b)}) circle (2pt) node [above left] {$f(b)$};
    % (b+db, f(b+db))の点
    \fill[magenta] (\b+\db,{f(\b+\db)}) circle (2pt) node [right] {$f(b+\Delta b)$};

    % x軸とy軸
    \draw [axis] (-0.5,0) -- (\b+\db+2,0) node (xaxis) [below] {$x$};
    \draw [axis] (0,-0.5) -- (0,5) node [left] {$y$};

    % dbの幅を表す矢印
    \draw [<->, magenta] ($(\b,-1.25em)+(0.2em,-1.25em)$) -- ($(\b+\db,-1.25em)+(-0.2em,-1.25em)$) node [below, midway, magenta] {$\Delta b$};
...
$b$を$\Delta b$だけ増加させたときに増える面積は、

\begin{equation}
  F(b+\Delta b) - F(b) = \int_{b}^{b+\Delta b} f(x) dx
\end{equation}

となるが、ここでさらに$\Delta b$を小さくしていくと…

増えた領域は、幅$db$、高さ$f(b)$の長方形とみなせるので、その面積は$f(b)db$となる。
...
\begin{center}
  \begin{tikzpicture}[
    ...
    \node [anchor=north,lnode] at (\a,0) {$a$};
    % 積分区間の上端のラベル
    \node[anchor=north,lnode, below right, xshift=-0.5em] at (\b,0) {$b \approx b+db$};

    % (b, f(b))の点
    \fill[magenta] (\b,{f(\b)}) circle (2pt) node [right] {$f(b) \approx f(b+db)$};
...
\begin{equation}
  F(b+db) = \origFn{F(b)} + \derivFn{f(b)}db
\end{equation}

この式は、定積分$F(b)$を$b$で微分すると、積分前の関数$f(x)$に戻るということを示している。
...
「微分したら元の関数に戻る」関数の1つが、前節で調べた$F(b) = \displaystyle\int_{a}^{b} f(x) dx$であったが、実はこのような関数は他にも存在する。

例えば、定数を微分すると$0$になるため、$F(b)$に任意の定数$C$を加えた関数$F(b) + C$を作っても、その微分結果は変わらず元の関数になる。
