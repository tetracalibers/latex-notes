...
\begin{equation}
  F(u+\Delta u) - F(u) = \int_{u}^{u+\Delta u} f(x) dx
\end{equation}
...
\begin{equation}
  F(u+du) = \origFn{F(u)} + \derivFn{f(u)}du
\end{equation}
...
「微分したら元の関数に戻る」関数の1つが、前節で調べた$F(u) = \displaystyle\int_{a}^{u} f(x) dx$であったが、実はこのような関数は他にも存在する。

例えば、定数を微分すると$0$になるため、$F(u)$に任意の定数$C$を加えた関数$F(u) + C$を作っても、その微分結果は変わらず元の関数になる。
