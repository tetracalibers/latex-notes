  ...
  \begin{minipage}{0.45\columnwidth}
    ...
    \scalebox{1.5}{
      \begin{tikzpicture}
        ...
        \draw[vector, dashed, very thick, SkyBlue] ($(S)+(\ax,0)$) -- ++(0,\ay) node[right, pos=0.6] {$\lambda_2 \vb*{a}_2$};

        % 基底ベクトル
        \draw[vector, very thick, Rhodamine] ([yshift=0cm]S) -- ++(1,0) node[below, pos=0.4] {$\vb*{a}_1$};
        \draw[vector, very thick, Cerulean] ([xshift=0cm]$(S)+(\ax,0)$) -- ++(0,0.5) node[right, near start] {$\vb*{a}_2$};
      \end{tikzpicture}
    }
  \end{minipage}
  \begin{minipage}{0.45\columnwidth}
